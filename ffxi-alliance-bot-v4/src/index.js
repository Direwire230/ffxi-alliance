...
import { buildAlliancePicker, buildEventComponents, buildEventEmbed, buildPartyPicker, buildSlotPicker, formatJobList, getUserJobs, normalizeJob, slotMatchesUserJobs } from './lib/render.js';
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'event') {
        ...
        if (sub === 'list') {
          ...
          const content = rows.length ? rows.map(e => `#${e.id} — **${e.name}** ${e.event_time || ''} ${e.locked ? '(locked)' : ''}`).join('\n') : 'No events found.';
      ...
      if (interaction.commandName === 'attendance') {
        ...
        const event = db.prepare('SELECT * FROM events WHERE id = ?').get(eventId);
        if (!event) return interaction.reply({ content: 'Event not found. Use /event list to find the event ID.', ephemeral: true });
    ...
    if (interaction.isButton()) {
      ...
      if (action === 'leave') {
        db.prepare('UPDATE event_slots SET user_id = NULL, display_name = NULL, signed_at = NULL WHERE event_id = ? AND user_id = ?').run(eventId, interaction.user.id);
      ...
      if (!isAdmin(interaction.member)) return interaction.reply({ content: 'Only admins can manage alliances.', ephemeral: true });
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (!slotMatchesUserJobs(slot.job, userJobs)) {
        return interaction.update({ content: `That slot requires **${slot.job}**, which does not match your saved jobs: **${formatJobList(userJobs)}**.`, components: buildSlotPicker(eventId, allianceNumber, partyNumber, interaction.user.id) });
      }

      const memberName = interaction.member?.displayName || interaction.user.username;
      const tx = db.transaction(() => {
        db.prepare('UPDATE event_slots SET user_id = NULL, display_name = NULL, signed_at = NULL WHERE event_id = ? AND user_id = ?').run(eventId, interaction.user.id);
        db.prepare('UPDATE event_slots SET user_id = ?, display_name = ?, signed_at = CURRENT_TIMESTAMP WHERE id = ?').run(interaction.user.id, memberName, slotId);
      });
      tx();
      await updateEventMessage(eventId);
      return interaction.update({ content: `Signed up for **A${slot.alliance_number} P${slot.party_number} S${slot.slot_number} - ${slot.job}**.`, components: [] });
    }
  } catch (err) {
    console.error(err);
